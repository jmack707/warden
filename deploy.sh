...
# External mode: OpenBao reaches your directory FROM INSIDE A CONTAINER, which does not
# read this host's /etc/hosts — Docker's embedded resolver only forwards to the
# nameservers in /etc/resolv.conf. A hostname that works for ldapsearch here can then
# fail as "no such host" four steps into the deploy. Catch it now, and offer the fix.
if ! warden_is_bundled; then
  case "${WARDEN_LDAP_HOST}" in
    *[!0-9.]*)
      if [ -z "${WARDEN_LDAP_HOST_IP:-}" ] &&
         grep -qiE "(^|[[:space:]])${WARDEN_LDAP_HOST}([[:space:]]|\$)" /etc/hosts 2>/dev/null; then
        echo "WARDEN_LDAP_HOST (${WARDEN_LDAP_HOST}) resolves through this host's /etc/hosts." >&2
        echo "  Containers never read /etc/hosts, so OpenBao would fail with \"no such host\"." >&2
        echo "  Set WARDEN_LDAP_HOST_IP=<its address> in .env and Warden will publish the name" >&2
        echo "  to the container; or make the name resolvable by a real nameserver." >&2
        exit 1
      fi;;
  esac
fi
...
LDAP_NEEDS_RESTART=""
# WARDEN_LDAP_HOST_IP publishes the directory's name inside the OpenBao container (see the
# /etc/hosts guard above). Rendered as a compose override so the main compose file stays
# free of site values; removed again when the variable is not set, so a stale entry from an
# earlier deploy cannot silently pin the name to an old address.
if [ -n "${WARDEN_LDAP_HOST_IP:-}" ] && ! warden_is_bundled; then
  cat > docker-compose.override.yml <<EOF
# GENERATED by deploy.sh from WARDEN_LDAP_HOST_IP in .env — do not edit.
services:
  openbao:
    extra_hosts:
      - "${WARDEN_LDAP_HOST}:${WARDEN_LDAP_HOST_IP}"
EOF
  echo "  publishing ${WARDEN_LDAP_HOST} -> ${WARDEN_LDAP_HOST_IP} inside the OpenBao container"
elif [ -f docker-compose.override.yml ] && head -1 docker-compose.override.yml | grep -q '^# GENERATED by deploy.sh'; then
