...
#
# The credential model is set by WARDEN_CRED_MODE in .env (see ADR 0006):
...
# an explicit inline WARDEN_CRED_MODE=… override wins over the .env default (preserve across source)
_MODE_IN="${WARDEN_CRED_MODE:-}"
set -a; . "${HERE}/../.env"; set +a
[ -n "$_MODE_IN" ] && WARDEN_CRED_MODE="$_MODE_IN"
