...
# One interface, two models selected by WARDEN_CRED_MODE — see ADR 0006.
#
...
#
#   ephemeral : a throwaway leased account is minted (username is random warden-…). The
...
WARDEN_CRED_MODE="${WARDEN_CRED_MODE:-ephemeral}"
WARDEN_EPHEMERAL_ROLE="${WARDEN_EPHEMERAL_ROLE:-warden-admin}"   # ldap/creds/<role> used in ephemeral mode

_cred_bao() { docker exec -i -e BAO_ADDR=http://127.0.0.1:8200 -e BAO_TOKEN="${BAO_TOKEN}" openbao bao "$@"; }

cred_issue() {
  local principal="${1:-}"
  case "$WARDEN_CRED_MODE" in
    ephemeral)
      _cred_bao read -format=json "ldap/creds/${WARDEN_EPHEMERAL_ROLE}" \
        | jq '{mode:"ephemeral", username:.data.username, password:.data.password, handle:.lease_id, ttl:.lease_duration}'
      ;;
    static)
      [ -n "$principal" ] || { echo "WARDEN_CRED_MODE=static requires a principal (CN)" >&2; return 2; }
      ...
      ;;
    *)
      echo "unknown WARDEN_CRED_MODE: '${WARDEN_CRED_MODE}' (want: ephemeral | static)" >&2; return 2
      ;;
  esac
}
