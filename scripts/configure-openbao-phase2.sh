...
echo "== write policy pua-apm-read ==" >&2
bao policy write pua-apm-read /pua/phase2-apm-policy.hcl >&2

echo "== mint scoped token (period 30m, renewable) ==" >&2
# display_name + period => a long-lived machine token bound only to pua-apm-read
TOKEN_JSON="$(bao token create -policy=pua-apm-read -period=30m -display-name=pua-apm -format=json)"
...
if [ "${GATE2A:-0}" = "1" ]; then
  ...
  echo "-- allowed: read ldap/creds/pua-admin --" >&2
  t read -format=json ldap/creds/pua-admin | jq -e '.data.username' >/dev/null \
  ...
  echo >&2; echo "GATE 2A PASSED — scoped token reads only ldap/creds/pua-admin." >&2
fi
