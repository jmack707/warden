...
# then run the PUA kill-switch on the pua-oss VM with BIGIP_PASS injected at runtime.
# The password is piped over ssh stdin (read via $(cat)), so it never lands in argv/ps.
#   Run on Nora:  bash bigip/run-revoke.sh --cn <CN> [--lease <id>] [--apm-key <k>] [--guac-id <uuid>]
set -euo pipefail
...
printf '%s' "$BPW" | ssh -o StrictHostKeyChecking=accept-new root@10.2.20.30 \
  "cd /root/pua-oss && BIGIP_PASS=\$(cat) ./scripts/revoke-all.sh $*"
